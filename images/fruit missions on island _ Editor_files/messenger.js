...
Messenger.prototype.connect = function(url) {
    ...
    // check if messenger server is available
    ...
    this._xhr.onreadystatechange = function() {
        if (this._xhr.readyState === 4) {
            ...
            if (this._xhr.status === 200) {
                // sockjs is available
                this.socket = new SockJS(this._url, null, { heartbeatTimeout: 60000 });
                this.socket.onopen = this._onopen.bind(this);
                this.socket.onmessage = this._onmessage.bind(this);
                this.socket.onerror = this._onerror.bind(this);
                this.socket.onclose = this._onclose.bind(this);
            } else {
                // not available, trigger reconnect
                this.reconnect();
            }
        }
    }.bind(this);

    // by requesting ../info resource of sockjs server
    this._xhr.open('GET', this._url + '/info', true);
    this._xhr.send(null);
};
